...
int main(int argc, char *argv[]){
  ...
  if (options[BM].count()){
    ...
    if (nbImages > 0) {
      double x = blockMatching(&(image_gray[0]),&(sample_gray[sampleNum]),2,resultBuffer[sampleNum],sampleNum);
      coord_j_min = (int) resultBuffer[sampleNum][0];
      coord_i_min = (int) resultBuffer[sampleNum][4];
      if (Show){
