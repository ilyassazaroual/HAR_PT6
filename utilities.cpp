...
void drawRectangle(cv::Mat *image,int coord_j_min, int coord_i_min,int bl_cols, int bl_rows){
  if (!image) return;

  Point matchLoc1(coord_j_min, coord_i_min);
  rectangle(*image, matchLoc1, Point( matchLoc1.x + bl_cols, matchLoc1.y + bl_rows), Scalar::all(255), 2, 8, 0 );
